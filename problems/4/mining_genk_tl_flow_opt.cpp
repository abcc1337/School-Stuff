...
int main() {
  ...
  for (int i = 0; i < xcnt; i++) {
    for (int j = 0; j < ycnt; j++) {
      ...
      if (hashes.find(hh) != hashes.end()) {
        index[i][j] = hashes[hh];
        c[2 * index[i][j] - 4] += (long long) q * (xs[i + 1] - xs[i]) * (ys[j + 1] - ys[j]);
        important[i][j] = false;
      } else {
        index[i][j] = hashes[hh] = V;
        add(V++, fin, (long long) q * (xs[i + 1] - xs[i]) * (ys[j + 1] - ys[j]));
        important[i][j] = true;
      }
    }
  }
  cerr << hashes.size() << " cells" << endl;
  for (int id = 0; id < t; id++) {
    add(0, V, n[id]);
    for (int i = 0; i < xcnt; i++) {
      for (int j = 0; j < ycnt; j++) {
